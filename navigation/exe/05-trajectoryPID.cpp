...
void computeTorques (const Vector6d& state, double& ul, double& ur, double& dt) {
	//MFU:
	double integralGainAng = 100;
	double integralGainLin = 100;
	...
	double u_x = (K(0)*error(0) + K(1)*error(1)) + integralGainLin * linIntErr;
	double u_spin = (K(2)*error(2) + K(3)*error(3)) + integralGainAng * angluarIntErr;
