...
public:
	...
	static void wheel_ctrl(krang_state_t *X, double dt);				

	/// Control arms with joint/workspace
	static void arm_ctrl(char* b, double* x, double* spnav, krang_state_t *X, double dt);

	/// Control arms to a goal position
	static void arm_goal(char* b, double* x, double* spnav, krang_state_t *X, double dt, bool left);
