...
void init_ft(somatic_motor_t* lwa, Vector6d* offset){
    ...
    computeOffset(imu, waist, *lwa, ft_data, *(world->getSkeleton(r_id)), *offset, true);
}
...
int main(int argc, char* argv[]) {
    ...
    Vector7d rdq, rvals;
    vector <int> rarm_ids;
    for(size_t i = 4; i < 17; i+=2) rarm_ids.push_back(i + 6);
    ...
    while(!somatic_sig_received) {
        ...
        somatic_motor_update(&daemon_cx, &rlwa);
        for(size_t i = 0; i < 7; i++) rvals(i) = rlwa.pos[i];
        world->getSkeleton(r_id)->setConfig(rarm_ids, rvals);
        ...
        bool result = (iter % ft_period == 0) && getFT(daemon_cx, ft_chan, rraw);
        if(!result) continue;

        Vector6d rideal = rraw + roffset;

        computeExternal(imu, waist, rlwa, rideal, *(world->getSkeleton(r_id)), rexternal, true);

        // Threshold the values - 4N for forces, 0.4Nm for torques
        if((rexternal.topLeftCorner<3,1>().norm() < 7) && 
           (rexternal.bottomLeftCorner<3,1>().norm() < 0.4)) {
            // somatic_motor_cmd(&daemon_cx, &rlwa, SOMATIC__MOTOR_PARAM__MOTOR_VELOCITY, dqZero, 7, NULL);
            continue;
        }
        ...
        wrenchToJointVels(rexternal, rdq, true);
        pv(rdq);
